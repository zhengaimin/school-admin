...
> 命名规范：通用规范统一参考 `context/naming.md`，本文仅补充索引文件相关规则。
...
| `APK_MESSAGE_TEMPLATE_AUDIT_ACTION` | APK 错误话术审计动作：update-更新，reset_inherit-恢复继承              | `string`  | `TApkMessageTemplateAuditActionValue` | `src/config/modules/common/apk-message-template.ts` |
...
- `APK_MESSAGE_TEMPLATE_AUDIT_ACTION_I18N` / `APK_MESSAGE_TEMPLATE_AUDIT_ACTION_OPTIONS` - APK 错误话术审计动作文案和选项
