...
- 常量/枚举导入（示例）：`@/constants/modules` 或 `@/config/modules`（命名规则见 `.agent/naming.md`，索引同步见 `.agent/index/constants.json`）
...
涉及枚举字段时，**必须**按 `.agent/naming.md` 的常量命名规则抽取常量（Enum/Type/I18N/Options），同步 `.agent/index/constants.json`，并在接口类型中引用对应的 `TxxxValue`。
...
2. 遵循 `.agent/naming.md` 的 Enum/Type/I18N/Options 命名结构（按项目需要可裁剪），并同步 `.agent/index/constants.json`
