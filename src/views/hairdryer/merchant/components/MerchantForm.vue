<script setup lang="ts" name="hairdryerMerchantForm">
...
/** 自定义证书文件上传函数 - 不实际上传，只保存 File 对象 */
const handleCertFileUpload = async (file: File) => {
  console.log("证书文件已选择:", file.name);
  certFile.value = file;
...
/** 自定义密钥文件上传函数 - 不实际上传，只保存 File 对象 */
const handleKeyFileUpload = async (file: File) => {
  console.log("密钥文件已选择:", file.name);
  keyFile.value = file;
...
const axiosGetMerchantConfig = async (): Promise<void> => {
  ...
  loading.value = true;
  try {
    const result = await getMerchantConfigApi(tenantId.value, DEVICE_TYPE.DRYER);
...
const handleReset = (): void => {
  form.value.deviceType = DEVICE_TYPE.DRYER;
