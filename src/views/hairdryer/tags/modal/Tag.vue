...
const acceptParams = (params: { title: string; type: string }, row?: TagRow) => {
  parameter.value = { ...parameter.value, ...params };

  if (params.type === "Add") {
    resetForm();
    currentSchoolName.value = storeSchoolName.value;
  } else if (params.type === "Edit" && row) {
...
<template>
  <el-dialog v-model="visible" :title="parameter.title" width="640px" destroy-on-close draggable align-center>
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="onSubmitForm(ruleFormRef)">确定</el-button>
    </template>
  </el-dialog>
</template>
