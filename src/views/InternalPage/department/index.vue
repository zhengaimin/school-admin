  ...
  <div class="table-box">
    ...
    <div class="table-list">
      <el-table class="my-custom-table" :data="carbonCk_list">
        ...
        <el-table-column label="级部名称" prop="name"> </el-table-column>
    ...
    <el-dialog v-model="dialogVisibleAdd" :close-on-click-modal="false" :title="form.id ? '编辑' : '新增'" :width="800">
      <div style="padding-left: 20px">
        <el-form ref="linkFormRef" :model="form" :rules="linkRules" class="demo-ruleForm" label-position="top">
          <el-row>
            <el-col :span="11">
              <el-form-item label="级部名称" prop="name">
...
export default {
  ...
  methods: {
    ...
    editRow(row) {
      this.dialogVisibleAdd = true;
      departmentsDetail({ id: row.id }).then(res => {
    ...
    confirmAdd() {
      this.$refs.linkFormRef.validate(valid => {
        if (valid) {
          if (this.form.id) {
            departmentsUpdate(this.form).then(res => {
              if (res.code == 0) {
          ...
          departmentsAdd(this.form).then(res => {
            if (res.code == 0) {
              this.dialogVisibleAdd = false;
    ...
    deleteRow(row) {
      ElMessageBox.confirm("确定删除该条数据吗?", "提示", {
        ...
        .then(() => {
          departmentsDelete({ id: row.id }).then(res => {
            if (res && res.code == 0) {
