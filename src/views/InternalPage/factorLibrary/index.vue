    ...
    <div class="right" ref="refRight">
      <div class="fossil-fuel">
        <div class="filter-box">
          ...
          <label>因子来源</label>
          <el-select style="width: 380px" multiple collapse-tags v-model="filterForm.source_ids" @focus="focusSource">
            <el-option v-for="v in industry_emission_list" :label="v.source_name" :value="v.source_id" :key="v.source_id">
              <span style="float: left">{{ v.source_name }}</span>
              <span style="float: right; font-size: 13px; color: var(--el-text-color-secondary)">
                {{ v.first_category }}{{ v.second_category }}
              </span>
            </el-option>
          </el-select>
          ...
          <div class="table-list">
            <el-table style="width: 100%; height: calc(100% - 50px)" class="my-custom-table" :data="tableData">
              ...
              <el-table-column label="单位" align="left">
                <template #default="{ row }">
                  {{ row.variety.unit.name }}
                </template>
              </el-table-column>
              <el-table-column label="密度" align="left">
                <template #default="{ row }">
                  {{ row.variety.density == 0 ? "/" : row.variety.density + " kg/m³" }}
                </template>
              </el-table-column>
              ...
              <el-table-column label="排放过程" align="left">
                <template #default="{ row }">
                  {{
                    ["", "化石燃料燃烧排放", "净购入的电力排放", "净购入的热力排放(热水)", "净购入的热力排放(蒸汽)"][
                      row.variety.emission_process_type
                    ]
                  }}
                </template>
              </el-table-column>
            ...
            <el-col :span="13">
              <el-form-item label="品种类型" prop="variety_type">
                <el-select v-model="linkRuleForm.variety_type" :disabled="linkRuleForm.id">
                  <el-option v-for="v in varityList" :label="v.name" :value="Number(v.id)" :key="v.id" />
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-row>
            <el-col :span="23">
              <el-form-item label="因子来源" prop="source_id">
                <el-select v-model="linkRuleForm.source_id" @focus="searchSource">
          ...
          <div class="form-box" v-if="formList.length > 0">
            <div v-for="v in formList" :key="v.sort">
              <el-form-item :label="v.name">
                <el-input v-model="linkRuleForm.fieldObj[v.key]"></el-input>
              </el-form-item>
            </div>
          </div>
        </el-form>
...
import {
  ...
  industry_emission_process_list,
  unit_list
...
export default {
  ...
  methods: {
    ...
    focusSource() {
      industry_emission_process_list().then(res => {
        if (res.code == 0 && res.data) {
          this.industry_emission_list = res.data[this.filterForm.emission_process_type];
        } else {
          this.industry_emission_list = [];
        }
      });
    },
    reset() {
    ...
    factor_list() {
      ...
      let str = `model_type=0&page=${this.page}&page_size=${this.page_size}&emission_process_type=${this.filterForm.emission_process_type}&source_ids=${source_ids_str}`;
    ...
    editRow(row) {
      ...
      this.$nextTick(() => {
        ...
        this.linkRuleForm.unit_id = row.variety.unit_id;
        this.linkRuleForm.density = row.variety.density;
    ...
    //获取指南-排放
    searchSource() {
      factor_source_list(`model_type=0`).then(res => {
