    ...
    <div v-for="(item, itemIndex) in formList" :key="item.id" class="table-box">
      <div class="btn-box">
        <div>
          ...
          <span class="source-type">化石燃料燃烧</span>
        ...
        <div>
          <div v-for="subItem in header" :key="subItem.sort">
            <div
              ...
              v-if="subItem.sort == 1 || (subItem.sort == 6 && itemCal.defaultAry[5] == 2)"
            >
              <div class="name">
                {{ subItem.name }}
...
export default {
      ...
      for (const item of this.reportList) {
        ...
        this.calList.map(item1 => {
          ...
          if (item.industry_varieties.variety_id == item1.industry_varieties_id && item.id == item1.boundary_id) {
            if (item1.data_json) {
              obj["ff_net_consumption"] = JSON.parse(item1.data_json)["ff_net_consumption"];
            }
  ...
  methods: {
    ...
    process_boundary_list() {
      process_boundary_list({ id: this.activeId, emission_process_type: 1 }).then(res => {
        ...
        this.$emit("fuelList", 1, this.reportList.length);
      });
    },
    //查核算表单
    report_list() {
    ...
    mathVal(itemCal, subItem) {
      if (subItem.key == "ff_elemental_carbon_content") {
        itemCal.formJson.ff_carbon_content_per_calorific_value =
          itemCal.formJson.ff_elemental_carbon_content / 100 / itemCal.formJson.ff_low_heat_output;
      }
    },
    ...
    get_emission_process_field_list() {
      emission_process_field_list(`emission_process_type=1`).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
