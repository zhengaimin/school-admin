    ...
    <div v-for="(item, itemIndex) in formList" :key="item.id" class="table-box">
      <div class="btn-box">
        <div>
          ...
          <span class="source-type">企业净购入的电力隐含的CO2排放</span>
          <span class="name-type">{{ item.boundary_type == 1 ? "直接排放" : "间接排放" }}</span>
          <span style="font-size: 12px; color: #8894b3">
            《{{ item.industry_varieties.factor_source.name }}》 {{ item.industry_varieties.factor_source.first_category }}
            {{ item.industry_varieties.factor_source.second_category }}
          </span>
        </div>
      ...
      <div v-for="itemCal in item.cal_list" :key="itemCal.id" class="form-box">
        ...
        <div>
          <div v-for="subItem in header" :key="subItem.sort">
            <div style="width: 250px; margin-right: 20px" v-if="subItem.sort == 1">
              <div class="name">
                {{ subItem.name }}
...
export default {
  ...
  computed: {
    formList() {
      ...
      let keys = [
        ...
        "cp_unit_of_measurement",
        "cp_carbon_emission_factor_unit",
        "cpncs_unit_of_measurement",
        "cpncs_deduction_factor_unit"
      ];
      for (const item of this.reportList) {
        ...
        this.calList.map(item1 => {
          ...
          if (item.industry_varieties.variety_id == item1.industry_varieties_id && item.id == item1.boundary_id) {
            if (item1.data_json) {
              obj["cpncs_net_consumption"] = JSON.parse(item1.data_json)["cpncs_net_consumption"];
            }
  ...
  methods: {
    ...
    process_boundary_list() {
      process_boundary_list({ id: this.activeId, emission_process_type: 6 }).then(res => {
        ...
        this.$emit("fuelList", 6, this.reportList.length);
      });
    },
    //查核算表单
    report_list() {
    ...
    get_emission_process_field_list() {
      emission_process_field_list(`emission_process_type=6`).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
