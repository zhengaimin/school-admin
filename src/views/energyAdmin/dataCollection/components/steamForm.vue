<template>
  <div style="margin-bottom: 20px">
    <div v-for="(item, itemIndex) in formList" :key="item.id" class="table-box">
      <div class="btn-box">
        <div>
          <span class="name"> {{ item.industry_varieties.variety.name }} </span>
          <span class="source-type">企业净购入的热力(蒸汽)隐含的CO2排放</span>
        ...
        <div>
          <div>
            <div style="width: 300px">
              <el-radio-group v-model="itemCal.defaultAry[0]">
                <el-radio value="1">热值</el-radio>
                <el-radio value="2">温度、压力</el-radio>
                <el-radio value="3">焓值</el-radio>
              </el-radio-group>
            </div>
          </div>
          <div v-for="subItem in header" :key="subItem.sort">
            <div
              ...
              v-if="
                (subItem.sort == 1 && (itemCal.defaultAry[0] == 2 || itemCal.defaultAry[0] == 3)) ||
                (subItem.sort == 2 && itemCal.defaultAry[0] == 2) ||
                (subItem.sort == 3 && itemCal.defaultAry[0] == 2) ||
                (subItem.sort == 4 && itemCal.defaultAry[0] == 3) ||
                (subItem.sort == 4 && itemCal.defaultAry[0] == 2) ||
                (subItem.sort == 5 && itemCal.defaultAry[0] == 1)
              "
            >
              <div class="name">
                {{ subItem.name }}
              ...
              <el-input-number
                :disabled="itemCal.defaultAry[0] == 2 && subItem.sort == 4"
                @blur="mathVal(itemCal, subItem)"
                style="width: 100%"
...
<script>
import { report_list, report_upsert, process_boundary_list, cal_data, cal_enthalpy } from "@/api/modules/dataCollection.js";
...
export default {
  ...
  computed: {
    formList() {
      this.header.map(v => {
        if (v.sort == 6) {
          v.key_unit = this.header[6].key;
        }
        if (v.sort == 8) {
          v.key_unit = this.header[8].key;
        }
      });
      let keys = [
      ...
      for (const item of this.reportList) {
        ...
        this.calList.map(item1 => {
          ...
          if (item.industry_varieties.variety_id == item1.industry_varieties_id && item.id == item1.boundary_id) {
            if (item1.data_json) {
              obj["nps_steam_heat_value"] = JSON.parse(item1.data_json)["nps_steam_heat_value"];
              obj["nps_steam_quality"] = JSON.parse(item1.data_json)["nps_steam_quality"];
              obj["nps_steam_temperature"] = JSON.parse(item1.data_json)["nps_steam_temperature"];
              obj["nps_steam_pressure"] = JSON.parse(item1.data_json)["nps_steam_pressure"];
              obj["nps_steam_enthalpy"] = JSON.parse(item1.data_json)["nps_steam_enthalpy"];
            }
  ...
  methods: {
    ...
    process_boundary_list() {
      process_boundary_list({ id: this.activeId, emission_process_type: 4 }).then(res => {
        ...
        this.$emit("fuelList", 4, this.reportList.length);
      });
    },
    //查核算表单
    report_list() {
    ...
    mathVal(itemCal, subItem) {
      if (subItem.key == "nps_steam_temperature" || subItem.key == "nps_steam_pressure") {
        let params = {
          steam_temperature: itemCal.formJson.nps_steam_temperature,
          steam_pressure: itemCal.formJson.nps_steam_pressure,
          instrument_id: itemCal.instruments_id
        };
        if (itemCal.formJson.nps_steam_temperature && itemCal.formJson.nps_steam_pressure) {
          cal_enthalpy([params]).then(res => {
            if (res.code == 0) {
              itemCal.formJson.nps_steam_enthalpy = res.data[0].enthalpy;
            }
          });
        }
      }
    ...
    confirmSubmit() {
      ...
      ary.map(item => {
        ary1.push({
          ...
          steam_type: Number(item.defaultAry[0]),
          year: this.year
        });
      });
    ...
    get_emission_process_field_list() {
      emission_process_field_list(`emission_process_type=4`).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
