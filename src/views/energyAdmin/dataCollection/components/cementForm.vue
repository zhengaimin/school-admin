...
export default {
      ...
      for (const item of this.reportList) {
        ...
        this.calList.map(item1 => {
          ...
          if (item.industry_varieties.variety_id == item1.industry_varieties_id && item.id == item1.boundary_id) {
            if (item1.data_json) {
              obj["cp_net_consumption"] = JSON.parse(item1.data_json)["cp_net_consumption"];
            }
  ...
  methods: {
    ...
    process_boundary_list() {
      process_boundary_list({ id: this.activeId, emission_process_type: 5 }).then(res => {
        ...
        this.$emit("fuelList", 5, this.reportList.length);
      });
    },
    //查核算表单
    report_list() {
    ...
    get_emission_process_field_list() {
      emission_process_field_list(`emission_process_type=5`).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
