<template>
  <div class="step-one">
    ...
    <div class="set-box table-box table-list" style="height: auto">
      ...
      <el-table :data="HList">
              ...
              <div v-if="activeId == row.id" class="update-input">
                <div v-if="row.radio0 == 2">
                  <el-input-number
                    ...
                    :precision="4"
                    v-model="row.emission_factor_json.ff_low_heat_output"
                  />
                </div>
                <div v-else>
              ...
              <div v-if="activeId == row.id" class="update-input">
                <div v-if="row.radio1 == 2">
                  <el-input-number
                    controls-position="right"
                    :precision="4"
                    v-model="row.emission_factor_json.ff_carbon_content_per_calorific_value"
                  />
                </div>
                <div v-else>
              ...
              <div v-if="activeId == row.id" class="update-input">
                <div v-if="row.radio2 == 2">
                  <el-input-number
                    controls-position="right"
                    :precision="4"
                    v-model="row.emission_factor_json.ff_fuel_carbon_oxidation_rate"
                  />
                </div>
                <div v-else>
        ...
        <el-table-column label="操作" align="center" width="100px">
          <template #default="{ row }">
            <div class="table-btn">
              <div @click="saveRow(row)" v-if="row.id == activeId">保存</div>
              <div @click="editRow(row)" v-else>
    ...
    <stepTwoValue :hlist="HList" :dlist="DList" :slist="SList" :nlist="NList" :flag="2" />
    <stepTwoWay
    ...
    />
    <div class="btn-box">
      <el-button @click="startMath(3, 1)">暂存</el-button>
      <el-button type="primary" @click="startMath(3, 2)">提交</el-button>
    </div>
    <div class="btn-box" style="margin-top: 1px; text-align: center; border-radius: 0 0 10px 10px">
      <el-button @click="startMath(0)">返回列表页</el-button>
      <el-button @click="startMath(1)">上一步</el-button>
    </div>
  </div>
</template>
<script>
...
import { product_carbon_footprint_data_list, product_carbon_footprint_data_fill } from "@/api/modules/carbonFootprint.js";
export default {
  components: {
    stepTwoValue,
    stepTwoWay
  ...
  methods: {
    ...
    startMath(val, type) {
      ...
      this.HList.map(v => {
        data_list.push({
          footprint_emission_type_id: v.id,
          data_json: JSON.stringify(v.emission_factor_json)
        });
      });
      this.DList.map(v => {
        data_list.push({
      ...
      let lca_list = [];
      this.lca_list.map(v => {
        if (v.unit_carbon_emission && v.data_source && this.checkList.indexOf(v.value) != -1) {
          lca_list.push({
            lca_id: v.id,
            lca_type: Number(v.value),
            unit_carbon_emission: Number(v.unit_carbon_emission),
      ...
      product_carbon_footprint_data_fill(params).then(res => {
        if (res.code == 0) {
          let str = type == 1 ? "暂存成功" : "提交成功";
