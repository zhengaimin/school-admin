...
import {
  ...
  grademessageratesDelete,
  grademessageratesDetail
...
export default {
  ...
  methods: {
    ...
    fetchTenantList() {
      ...
      grademessageratesList(params).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
    ...
    editRow(row) {
      this.dialogVisibleAdd = true;
      grademessageratesDetail({ rateTemplateID: row.rateTemplateID }).then(res => {
    ...
    confirmAdd() {
      this.$refs.linkFormRef.validate(valid => {
        if (valid) {
          if (this.form.rateTemplateID) {
            grademessageratesUpdate(this.form).then(res => {
              if (res.code == 0) {
          ...
          grademessageratesAdd(this.form).then(res => {
            if (res.code == 0) {
              this.dialogVisibleAdd = false;
    ...
    deleteRow(row) {
      ElMessageBox.confirm("确定删除该条数据吗?", "提示", {
        ...
        .then(() => {
          grademessageratesDelete({ id: row.id }).then(res => {
            if (res && res.code == 0) {
