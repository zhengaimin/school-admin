  ...
  <div class="fossil-fuel">
    <div class="filter-box">
      ...
      <el-date-picker
        ...
        placeholder="请选择年份"
      />
      <span class="total-val-box">
        全年能源预算总量：
        <span> {{ companyObj.annual_energy_budget }} /tce </span>
      </span>
    </div>
    <div class="content">
      <h1>
...
export default {
  ...
  methods: {
    ...
    focusAction(row, type) {
      ...
      if (type == 1) {
        this.productListTable.map(v => {
          yearVal += v.month_list[0] ? v.month_list[0] : 0;
        });
      } else if (type == 2) {
    ...
    get_product_energy_list() {
      product_energy_list(`year=${this.year}&&budget_type=1`).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
    ...
    get_dept_energy_list() {
      dept_energy_list(`year=${this.year}&&budget_type=1`).then(res => {
        if (res.code == 0 && res.data && res.data.list) {
