<script setup lang="ts" name="videoRateSchoolMessageForm">
...
import { getSchoolMessageRateApi, putSchoolMessageRateApi } from "@/api/modules";
...
const fetchData = async (): Promise<void> => {
  ...
  loading.value = true;
  try {
    const result = await getSchoolMessageRateApi(Number(props.schoolId));
...
const handleSubmit = async (): Promise<void> => {
  ...
  try {
    await putSchoolMessageRateApi(Number(props.schoolId), {
      rate: form.rate,
...
<template>
  <div v-loading="loading" class="h-full w-full flex flex-col rounded-lg overflow-hidden">
