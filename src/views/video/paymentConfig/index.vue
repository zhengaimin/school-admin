<script setup lang="ts" name="videoPaymentConfig">
import type { FormInstance, FormRules } from "element-plus";
import type { IVideoPaymentConfigForm } from "./utils/payload";

import { computed, onMounted, reactive, ref } from "vue";
import { ElMessage } from "element-plus";
import { getTenantPaymentConfigApi, putTenantPaymentConfigApi } from "@/api/modules";
import { useUserStore } from "@/stores/modules/user";
import { buildPutTenantPaymentConfigPayload } from "./utils/payload";
...
/** 获取初始表单数据 */
function getInitialFormData(): IVideoPaymentConfigForm {
  return {
    fixedAmounts: "",
    minAmount: null,
    maxAmount: null,
...
/** 表单数据 */
const form = reactive<IVideoPaymentConfigForm>(getInitialFormData());
...
function normalizeFixedAmounts(value: unknown): string {
  ...
  try {
    ...
    return Array.isArray(amountList) ? amountList.join(",") : text;
  } catch (error) {
  ...
  try {
    ...
    if (result.code === 0 && result.data) {
      Object.assign(form, getInitialFormData(), {
        fixedAmounts: normalizeFixedAmounts(result.data.fixedAmounts),
        minAmount: result.data.minAmount ?? null,
        maxAmount: result.data.maxAmount ?? null,
...
async function handleSubmit(): Promise<void> {
  ...
  const payload = buildPutTenantPaymentConfigPayload(form);
