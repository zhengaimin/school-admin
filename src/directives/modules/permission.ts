...
const hasPermission = (value: string | string[] | null | undefined, permissions: string[], roleLevel?: TRoleLevelValue) => {
...
const permission: Directive = {
  mounted(el, binding) {
    ...
    const allowed = hasPermission(binding.value, permissionStore.permissionCodesGet ?? [], userStore.userInfo?.roleLevel);
    if (!allowed) {
      removeElement(el);
    }
  },
